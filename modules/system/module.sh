...
system::preview() {
    ...
    if [[ "${PROFILE_SYSTEM_FIREWALLD:-true}" == "true" ]]; then
        ...
        if systemctl is-active --quiet firewalld 2>/dev/null; then
            echo "  Firewall: active → disable  [REVERT]"
        else
            echo "  Firewall: already inactive  [OK]"
        fi
    fi
    ...
    if [[ "${PROFILE_SYSTEM_BLUETOOTH:-true}" == "true" ]]; then
        ...
        if systemctl is-enabled --quiet bluetooth 2>/dev/null; then
            echo "  Bluetooth: enabled → disable  [REVERT]"
        else
    ...
    if [[ "${PROFILE_SYSTEM_TUNED:-true}" == "true" ]]; then
        ...
        if systemctl is-enabled --quiet tuned 2>/dev/null; then
            echo "  Tuned: enabled → disable  [REVERT]"
        else
            echo "  Tuned: already disabled  [OK]"
        fi
    fi
...
system::apply() {
    ...
    if [[ "${PROFILE_SYSTEM_FIREWALLD:-true}" == "true" ]]; then
        ...
        if systemctl is-active --quiet firewalld 2>/dev/null; then
            ...
            ok "firewalld: disabled (reverted)"
        fi
    fi
    ...
    if [[ "${PROFILE_SYSTEM_BLUETOOTH:-true}" == "true" ]]; then
        ...
        if systemctl is-enabled --quiet bluetooth 2>/dev/null; then
            ...
            ok "Bluetooth: disabled (reverted)"
        fi
    fi
